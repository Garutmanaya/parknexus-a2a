...
if [[ ! -f .env ]]; then
  echo "No .env found. Using .env.demo defaults..."
  cp .env.demo .env
fi

# Load .env into this shell so POSTGRES_ADMIN_PASSWORD and other vars are available.
set -a
source .env
set +a
...
if [[ -z "${PG_BIN}" || ! -x "${PG_BIN}/initdb" ]]; then
  ...
  echo "Installed PostgreSQL files:"
  find /usr/lib/postgresql -maxdepth 4 -type f | sort || true
  exit 1
fi

echo "Using PostgreSQL binaries from: ${PG_BIN}"

# Demo mode: PostgreSQL is ephemeral.
