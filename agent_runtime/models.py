...
class Garage(Base):
    ...
    garage_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
...
class ParkingSlot(Base):
    ...
    garage_id: Mapped[str] = mapped_column(ForeignKey("garages.garage_id"), nullable=False)
...
class SlotHold(Base):
    ...
    slot_id: Mapped[str] = mapped_column(ForeignKey("parking_slots.slot_id"), nullable=False)
    user_id: Mapped[str] = mapped_column(String(100), nullable=False)
    status: Mapped[ReservationStatus] = mapped_column(Enum(ReservationStatus), default=ReservationStatus.HELD, nullable=False)
...
class Reservation(Base):
    ...
    reservation_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    slot_id: Mapped[str] = mapped_column(ForeignKey("parking_slots.slot_id"), nullable=False)
    hold_id: Mapped[str | None] = mapped_column(ForeignKey("slot_holds.hold_id"), nullable=True)
    user_id: Mapped[str] = mapped_column(String(100), nullable=False)
    status: Mapped[ReservationStatus] = mapped_column(Enum(ReservationStatus), default=ReservationStatus.CONFIRMED, nullable=False)
...
if __name__ == "__main__":
    logger.info("Loaded provider ORM models")
