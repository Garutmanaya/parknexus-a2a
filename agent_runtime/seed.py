...
def get_or_create_provider(db, agent_config: dict) -> Provider:
    provider = db.query(Provider).filter(Provider.agent_id == agent_config["agent_id"]).first()
    if provider:
        return provider
    provider = Provider(
    ...
    db.add(provider)
    db.flush()
    logger.info("provider_metadata_created agent_id=%s", agent_config["agent_id"])
    return provider
...
def get_or_create_garage(db, provider: Provider, agent_config: dict) -> Garage:
    ...
    db.add(garage)
    db.flush()
    logger.info("garage_created agent_id=%s garage=%s", agent_config["agent_id"], garage.name)
    return garage
...
def resolve_pricing(agent_config: dict, level_name: str) -> tuple[Decimal, Decimal | None, Decimal | None]:
    ...
    if isinstance(level_pricing, dict):
        hourly = level_pricing.get("hourly_rate", pricing.get("default_hourly_rate", 10.0))
        ...
        hourly = level_pricing or pricing.get("default_hourly_rate", 10.0)
...
def resolve_slot_features(agent_config: dict, level_name: str, row_label: str, column_number: int) -> tuple[SlotType, bool, bool]:
    features = agent_config.get("features", {})
    ev_slots = {parse_slot_ref(slot_ref) for slot_ref in features.get("ev_slots", [])}
    handicap_slots = {parse_slot_ref(slot_ref) for slot_ref in features.get("handicap_slots", [])}
...
def slot_exists(db, slot_code: str) -> bool:
    return db.query(ParkingSlot).filter(ParkingSlot.slot_code == slot_code).first() is not None


def generate_slot_code(agent_id: str, level_name: str, row_label: str, column_number: int) -> str:
    ...
    for level in layout["levels"]:
        ...
        for row in level["rows"]:
            ...
            for column_number in range(1, columns + 1):
                slot_code = generate_slot_code(agent_config["agent_id"], level_name, row_label, column_number)
                if slot_exists(db, slot_code):
                    continue
                slot_type, ev_charger, handicap = resolve_slot_features(agent_config, level_name, row_label, column_number)
                db.add(ParkingSlot(
...
def seed_provider_database(agent_config: dict) -> None:
    logger.info("provider_seed_started agent_id=%s", agent_config["agent_id"])
    ...
    try:
        ...
        db.commit()
        logger.info("provider_seed_completed agent_id=%s garage=%s inserted_slots=%s", agent_config["agent_id"], garage.name, inserted_count)
    except Exception:
        db.rollback()
        logger.error("provider_seed_failed agent_id=%s", agent_config.get("agent_id"), exc_info=True)
        raise
    finally:
        db.close()
...
if __name__ == "__main__":
    ...
    seed_provider_database(load_agent_config(args.config))
